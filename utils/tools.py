...
#     # config_dir = os.path.join("/666/ICASSP-CVAE/CVAE-TTS/config", dataset)
...
#         logger.add_scalar("Loss/encoder_loss", losses[4], step)
...
# def synth_one_sample(targets, predictions, inference, vocoder, mel_stats, model_config, preprocess_config, step):
...
#     mel_inference = inference[1][0,:,:].detach().transpose(0,1)
#     attention = predictions[3][0, :reduced_mel_len, :src_len].detach().transpose(0, 1) # [seq_len, mel_len]
#     attention_inference = inference[3][0,:,:].detach().transpose(0, 1) # [seq_len, mel_len]
...
#             mel_inference.cpu().numpy(),
#             attention.cpu().numpy(),
#             attention_inference.cpu().numpy(),
#         ],
#         ["Synthetized Spectrogram", "Ground-Truth Spectrogram", "Inferenced Spectrogram", "Decoder Alignment", "Decoder Alignment - inference"],
#         attention=True,
#     )
...
#             preprocess_config,
#         )[0]
#         wav_inference = vocoder_infer(
#             mel_inference.unsqueeze(0),
#             vocoder,
#             model_config,
#             preprocess_config,
#         )[0]
#     else:
#         wav_reconstruction = wav_prediction = wav_inference = None

#     return fig, gate_fig, wav_reconstruction, wav_prediction, wav_inference, basename
...
#     if args.mode == "single":
#         basename = args.basename + "_" + basename
#     elif args.mode == "batch":
#         basename = "batch_" + str(targets[2][0].item()) + "_" + basename
#     elif args.mode == "sample":
#         basename = args.accent + "_" + basename
...
#         path, str(args.restore_step), "img{}_{}.png".format(args.speaker_id, basename)\
#              if multi_speaker and (args.mode == "single" or args.mode == "sample" or args.mode == "sample_stats") else "{}.png".format(basename)))
#     plt.close()
...
#         path, str(args.restore_step), "{}_{}.wav".format(args.speaker_id, basename)\
#              if multi_speaker and (args.mode == "single" or args.mode == "sample" or args.mode == "sample_stats") else "{}.wav".format(basename)),
...
#         if (i == len(data)-2 and attention) or (i == len(data)-1 and attention):
...
def pad_1D(seqs, pad_value=0):
    ...
    padded = torch.zeros(len(seqs), max_len, dtype=torch.long) + pad_value
    for i, seq in enumerate(seqs):
        padded[i, : len(seq)] = torch.as_tensor(seq, dtype=torch.long)
    return padded           # [B, max_len]

def pad_2D(mels, pad_value=0.0):
    # mels: list of [n_mels, T_i]  (torch or np)
    n_mels = mels[0].shape[0]
    max_len = max(m.shape[1] for m in mels)
    padded = torch.zeros(len(mels), n_mels, max_len) + pad_value
    for i, mel in enumerate(mels):
        mel_tensor = torch.as_tensor(mel)
        padded[i, :, : mel_tensor.shape[1]] = mel_tensor
    return padded           # [B, n_mels, max_len]